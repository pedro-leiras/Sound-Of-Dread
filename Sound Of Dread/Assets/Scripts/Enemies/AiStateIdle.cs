...
public class AiIdleState : AiState{
    ...
    public void Enter(AiAgent agent){
        ...
        agent.navMeshAgent.destination = agent.navMeshAgent.destination;
    }
    ...
    public void Update(AiAgent agent){
        ...
        if (timer >= agent.animator.GetCurrentAnimatorStateInfo(0).length - 1.0f) agent.stateMachine.ChangeState(AiStateId.Patrol);
